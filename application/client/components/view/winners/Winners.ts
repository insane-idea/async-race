...
import winnersHTML from './content';
...
class Winners {
  ...
  static async getWinnersJSON(winnersURL: string): Promise<WinnerType[]> {
    return (await fetch(winnersURL)).json();
  }

  static async renderWinners(page: number): Promise<void> {
    const winnersContainer = document.querySelector('.winners-body') as HTMLElement;
    const winnersCount = document.querySelector('.winners-amount') as HTMLElement;
    const winnersPageNum = document.querySelector('.winners-page-num') as HTMLElement;

    await Winners.API.getWinners(page, 10, 'id', 'ASC')
      .then((winnersJSON) => {
        winnersCount.innerHTML = winnersJSON.count.toString();
        winnersPageNum.innerHTML = State.winnersPageNum.toString();
        return winnersJSON.items;
      })
      .then((winnersItems) => {
        winnersContainer.innerHTML = '';
        if (winnersItems.length > 0) {
          const arrayOfPromises: Promise<void>[] = winnersItems.map((winnerData, index) =>
            Winners.drawWinner(winnerData, index)
          );
          Promise.all(arrayOfPromises);
        }
      })
      .then(() => Winners.utils.switchWinnersPaginationButtons());
  }

  static async drawWinner(winnerData: WinnerType, index: number): Promise<void> {
    await Winners.API.getCar(winnerData.id).then((car) => {
      if (car && car.color) {
        const winnersContainer = document.querySelector('.winners-body') as HTMLElement;
        const carSvg = `
        ...
        const winner = document.createElement('tr') as HTMLElement;
        ...
        winner.innerHTML = `
                        ...
                        <td class="wins-amount">${winnerData.wins}</td>
                        <td class="best-time">${winnerData.time}</td>
                    `;
        winnersContainer.append(winner);
      }
    });
  }

  async winnersPaginationListener(): Promise<void> {
    ...
    prevWinnersBtn.addEventListener('click', () => {
      if (!prevWinnersBtn.disabled) {
        State.winnersPageNum = +State.winnersPageNum - 1;
        Winners.renderWinners(State.winnersPageNum).then(() => Winners.utils.switchWinnersPaginationButtons());
      }
    });

    nextWinnersBtn.addEventListener('click', () => {
      if (!nextWinnersBtn.disabled) {
        State.winnersPageNum = +State.winnersPageNum + 1;
        Winners.renderWinners(State.winnersPageNum).then(() => Winners.utils.switchWinnersPaginationButtons());
      } else {
        prevWinnersBtn.disabled = State.winnersPageNum === 1;
      }
    });
  }

  async changePageNum(): Promise<void> {
    const winnersContainer = document.querySelector('.winners-table') as HTMLElement;

    if (winnersContainer) {
      await Winners.utils.switchWinnersPaginationButtons().then(() => {
        this.winnersPaginationListener();
      });
    }
  }

  async run(): Promise<void> {
    await Winners.renderHTML()
      .then(() => Winners.renderWinners(State.winnersPageNum))
